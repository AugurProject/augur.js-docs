...
```
Provides JavaScript bindings for the [Cash Solidity Contract](https://github.com/AugurProject/augur/blob/master/packages/augur-core/source/contracts/Cash.sol), which is used internally by Augur as an [ERC-20](https://eips.ethereum.org/EIPS/eip-20) and [ERC-777](https://eips.ethereum.org/EIPS/eip-777) wrapper for ETH.
...
Gets the remaining amount of [REP](#rep) needed to fill the [Dispute Bond](#dispute-bond) for this [Crowdsourcer](#crowdsourcer).
...
augur.api.DisputeWindow.getWindowId({ 
  tx: { to: disputeWindow } 
...
augur.api.Market.getHighestNonTentativeParticipantStake({ 
   tx: { to: market } 
}, function (error, highestNonTentativeParticipantStake) { 
  console.log(highestNonTentativeParticipantStake); 
});
// example output:
!!!TBD!!! Add example output
...
### augur.api.Market.getHighestNonTentativeParticipantStake(p, callback)

Returns the highest amount of attoREP staked in in the Market's InitialReporter/DisputeCrowdsourcer that is not the [Tentative Outcome](#tentative-outcome).
...
The Reputation Token, or REP, is an ERC-20 token that implements all of the required functions listed in the [ERC-20 Token Standard](https://eips.ethereum.org/EIPS/eip-20). It does not, however, implement the optional functions. It also implements the [ERC-777 Token Standard](https://eips.ethereum.org/EIPS/eip-777).
...
The Share Token is an ERC-20 token that implements all of the required functions listed in the [ERC-20 Token Standard](https://eips.ethereum.org/EIPS/eip-20). It does not, however, implement the optional functions. It also implements the [ERC-777 Token Standard](https://eips.ethereum.org/EIPS/eip-777). Within Augur, it represents [Shares](#share) in [Market](#market) [Outcomes](#outcome).
...
Returns the [Dispute Window](#dispute-window) ID for the [Universe](#universe) at the specified timestamp. This ID is calculated by dividing the timestamp by the [Universe's](#universe) Dispute Window duration in seconds. If the Dispute Window is an Initial Window (1 day in duration), an additional buffer is added to the ID.
...
Returns whether the specific [Universe](#universe) is a container for a given Share Token. ([Shares](#share) are represented within Augur's smart contracts as [ERC-20](https://eips.ethereum.org/EIPS/eip-20)/[ERC-777](https://eips.ethereum.org/EIPS/eip-777) tokens called Share Tokens.)
